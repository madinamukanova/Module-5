...
$sqli = "SELECT * from worker
       where WorkerID='$WorkerID'";
$rw = mysqli_query($conn,$sqli);
$total_rows=mysqli_num_rows($rw);
if($total_rows> 0){
$sql = "INSERT INTO passwordrecovery (WorkerID)
VALUES ('$WorkerID')";
$run = mysqli_query($conn,$sql);
$sqla = mysqli_query($conn, "SELECT WorkerPassword FROM worker WHERE WorkerID = '$WorkerID'");
...
<form action = "forgotpasswordphp.php" method = "POST">
<p style = "text-align:left;color:#E58E0C;;font-size:40px;position:absolute;top:270px;left:510px;">Password Recovery</p>
<?php 
while ($result = mysqli_fetch_array($sqla)){
?>
<p style = "text-align:left;color:blue;font-size:20px;position:absolute;top:340px;left:530px;">Your password is <?php echo base64_decode($result['WorkerPassword']); ?></p>
<?php 
}
}
else {
header("location:unsuccessfulrecovery.php");
}
}
?>
<input type="text" name="WorkerID" size="35" id = "id" style="padding:10px;position:absolute;top:390px;left:520px;"
maxlength="30" placeholder = "Enter your unique ID..." pattern="[0-9]+" title="Please enter digits only" required />


<input type = "submit" name = "submit" value = "Send" style = "background-color:red;color:white;height: 50px;margin:15px;font-family:sans serif;font-size:30px;position:absolute; right:530px;top:420px;" onclick = "MyFunction1()">
...
if (id==""){
alert("Error. You must enter your Worker ID.");
}
else{
alert("Now we will check whether your Worker ID exists in the database.");
